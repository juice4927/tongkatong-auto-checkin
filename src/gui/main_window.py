...
class MainWindow(QMainWindow):
    ...
    def _create_log_tab(self):
        ...
        self.log_viewer_widget = LogViewerWidget(base_dir=base_dir)
        ...
        class _ConnWorker(QThread):
            ...
            def run(self):
                ...
                for port in port_candidates:
                    try:
                        auto = UIAutomator2Impl(host=self._host, port=port)
                        auto.connect()
                ...
                try:
                    ...
                    for port in port_candidates:
                        try:
                            ...
                            continue
                    self.done.emit(False, f"连接失败：已尝试端口 {port_candidates}", None)
                except Exception as e:
                ...
                try:
                    ...
                    if hasattr(self._automator, "set_makeup_windows"):
                        self._automator.set_makeup_windows(
                            CheckinOrchestrator.makeup_windows_from_config(self._config)
                        )
