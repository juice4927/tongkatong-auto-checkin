...
class TimeConfigWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        makeup_hint = QLabel("启动时若打卡时间已过但仍在窗口内会自动补打；结束时 > 23 表示次日，如 28:00 = 次日 04:00")
    ...
    def _reset_to_default(self):
        ...
        mw = default_config.makeup_window
        defaults = {
            'morning_signin': mw.morning_signin,
            'morning_signout': mw.morning_signout,
            'afternoon_signin': mw.afternoon_signin,
    ...
    def _validate_inputs(self) -> tuple[bool, str]:
        ...
        for key, spins in self._makeup_spins.items():
            ...
            if end < start:
                ...
                return False, f"补签窗口 {name}：结束时间不能早于开始时间（跨日请把结束时设为 >23，例如 28:00）"
            if end == start:
                name = label_map.get(key, key)
                return False, f"补签窗口 {name}：结束时间不能等于开始时间"
