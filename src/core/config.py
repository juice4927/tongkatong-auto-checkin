...
class MakeupWindowConfig(BaseModel):
    """补签时间窗口配置（格式：[起始时, 起始分, 结束时, 结束分]，结束时>23表示次日）"""
    ...
    def _validate_window(cls, v: list[int]) -> list[int]:
        if len(v) != 4:
            raise ValueError("补签窗口格式必须为 [起始时, 起始分, 结束时, 结束分]")
        sh, sm, eh, em = v
        if not (0 <= sh <= 48 and 0 <= eh <= 48):
            raise ValueError("补签窗口小时必须在 0..48 范围内")
        if not (0 <= sm <= 59 and 0 <= em <= 59):
            raise ValueError("补签窗口分钟必须在 0..59 范围内")
        return v
...
class ConfigManager:
    ...
    def get_default_config(self) -> Config:
        ...
        config = Config(**self._load_default_data())
