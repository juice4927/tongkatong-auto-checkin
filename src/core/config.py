...
APP_DATA_DIR_NAME = "TongKaTong"


def get_app_data_dir() -> Path:
    """返回稳定的用户数据目录，避免 Velopack 版本目录变化覆盖配置。"""
...
class ConfigManager:
    ...
    def __init__(self, config_dir: Path = None):
        if config_dir is None:
            self.config_dir = get_runtime_root() / "config"
        else:
    ...
    def _migrate_legacy_runtime_config(self) -> None:
        ...
        if self.config_file.exists():
            return
        legacy_config_file = Path(sys.executable).parent / "config" / "user_config.json"
        if legacy_config_file == self.config_file or not legacy_config_file.exists():
            return
        try:
            self.config_dir.mkdir(parents=True, exist_ok=True)
            self.config_file.write_text(legacy_config_file.read_text(encoding="utf-8"), encoding="utf-8")
