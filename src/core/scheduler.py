...
class CheckinOrchestrator:
    ...
    def __init__(self, automator, holiday_checker, config_manager):
        ...
        self._daily_result_meta = []

    @staticmethod
    def checkin_windows_from_config(cfg) -> dict:
        """从主配置提取有效打卡窗口字典，供自动和手动打卡共用。"""
        mw = cfg.checkin_window
        return {
    ...
    @staticmethod
    def makeup_windows_from_config(cfg) -> dict:
        """兼容旧调用名；语义已改为有效打卡窗口。"""
        return CheckinOrchestrator.checkin_windows_from_config(cfg)
    ...
    def get_daily_success_stats(self) -> dict:
        """Return today's terminal check-in success statistics."""
        with self._results_lock:
            total = len(self._daily_results)
            success_count = sum(1 for _, success, _, _ in self._daily_results if success)
            failed = total - success_count
            failure_codes = {}
            for meta in self._daily_result_meta:
                if meta.get("success"):
                    continue
                code = str(meta.get("failure_code", "")).strip() or "unknown"
    ...
    def _record_terminal_result(
        ...
        with self._results_lock:
            ...
            self._daily_result_meta.append(dict(self._last_result_meta))
    ...
    def _classify_and_schedule_jobs(self, action_map: dict, checkin_configs: dict) -> dict:
        ...
        """
        now = datetime.now()
        mw = self.config_manager.config.checkin_window
        makeup_windows = {
            'morning_signin':   mw.morning_signin,
            'morning_signout':  mw.morning_signout,
            'afternoon_signin': mw.afternoon_signin,
        ...
        for job_id, run_time in self._checkin_times.items():
            ...
            if in_window:
                logger.info(f"[启动检查] {label} 时间已过，在有效打卡窗口内，将尝试补打")
                pending_makeup.append((action, job_id, label))
            else:
                logger.info(f"[启动检查] {label} 时间已过且超出有效打卡窗口，跳过")
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if cfg.mumu.gps_latitude and cfg.mumu.gps_longitude and not gps_set_ok:
                    ...
                    return
                if hasattr(self.automator, "set_makeup_windows"):
                    self.automator.set_makeup_windows(self.checkin_windows_from_config(cfg))
