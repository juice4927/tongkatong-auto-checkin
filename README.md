...
2. 确认 MuMu 已开启 ADB 调试；ADB 路径可留空，程序会优先从填写的 MuMu 安装目录自动查找。
3. 手动在目标 App 里走一遍 `工作台 -> 考勤`，确认页面路径没有变化。
4. 安装项目依赖并启动程序。
5. 在设置页填写模拟器地址、端口、MuMu 安装目录、打卡 GPS 坐标和目标 App 包名；ADB 路径只有自动查找失败时才需要手动选择。
6. 点击 `测试连接`，确认设备联通正常。
7. 进入时间配置页，设置四个打卡点和随机范围。
8. 根据需要开启节假日跳过、通知、守护恢复，并配置有效打卡窗口。打卡时间配置只决定随机触发时间，有效打卡窗口用于判断页面已有时间是否正常。
...
也可以推送版本标签触发 GitHub Actions 自动打包并发布：
...
卸载请打开 Windows「设置 → 应用 → 已安装的应用」，找到「通卡通」或「TongKaTong」后选择卸载。
